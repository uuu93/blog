package org.lxb.framework.entity;
 ...
 * 分页类
 * 
 * @author FH QQ 313596790[青苔] 创建时间：2014年6月28日
 */
public class Page
{
	...
	private String frontPageStr; // PC前台分页样式显示
	private PageData pd = new PageData();
	/**
	 * 是否分页:默认分佣,后台查询的时候可能需要分页,但是导出报表的时候不需要分页,此参数可以不让其分页
	 */
...
//		{
			...
			if (currentPage == 1)
			{
				...
				sb.append("	<li><input type=\"number\" value=\"\" id=\"toGoPage\" style=\"width:55px;text-align:center;float:left\" placeholder=\"页码\" min=\"1\"/></li>\n");
			...
			else
			{
				...
				sb.append("	<li><a>共<font color=red>" + totalResult + "</font>条</a></li>\n");
				sb.append("	<li><input type=\"number\" value=\"\" id=\"toGoPage\" style=\"width:50px;text-align:center;float:left\" placeholder=\"页码\" min=\"1\"/></li>\n");
				sb.append("	<li style=\"cursor:pointer;\"><a onclick=\"toTZ();\"  class=\"btn btn-mini btn-success\">跳转</a></li>\n");
...
//		}
	...
	public Map<String, String> tidyPage()
	{
		Map<String, String> map = new HashMap<String, String>();
		map.put("totalPage", String.valueOf(totalPage));
		map.put("currentPage", String.valueOf(currentPage));
		map.put("totalResult", String.valueOf(totalResult));
		return map;
	}
	...
	public String getFrontPageStr()
	{
		...
		if (totalResult > 0 && totalPage > 1)
		{
			...
			{
				if (currentPage == i)
					sb.append("<a class='active' href='javascript:;' onclick=\"nextPage(" + i + ")\">" + i + "</a>");
				else
			...
			{
				sb.append("<span>...</span>");
				sb.append("<a href='javascript:;' onclick=\"nextPage(" + totalPage + ")\">" + totalPage + "</a>");
			}
			sb.append("<p>到第<input type=\"number\" value=\"\" id=\"toGoPage\"  placeholder=\"\" />页</p>");
			sb.append("<a class='realbtn' href='javascript:;' onclick=\"toTZ();\">确定</a>");
		}
