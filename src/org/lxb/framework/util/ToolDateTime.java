...
public class ToolDateTime
{
	...
	{
		int sec = (int) ((end.getTime() - start.getTime()) / 1000);
		return sec;
	}
	 ...
	 * @param format
	 *          日期格式
	 * @return 根据格式返回第weeknum周的最后一天
	 */
	public static String getWeeksEndDate(String weeknum, String format)
	{
		int num = 1;
		try
		{
			num = Integer.parseInt(weeknum);// 默认第一周
		}
		catch (Exception e)
		{
			e.printStackTrace();
		}
		if ((!pattern_ymd.equals(format)) || (!pattern_ymd_hms.equals(format)) || (!pattern_ymd_hms_s.equals(format)))
		{
			format = pattern_ymd;// 默认格式
		}
		...
		return simpleDateFormat.format(calcDate);
	}
	 ...
	 * @return
	 */
	public static Long transferStringDateToLong(String pattern, String date)
	{
		...
		catch (ParseException e)
		{
			...
			// e.printStackTrace();
			log.error("transferStringDateToLong.parse异常：date值" + date + "，pattern值" + pattern);
		}
		if (null == dt)
		{
			dt = new Date();
