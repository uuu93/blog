						...
						<div id="zhongxin" style="padding-top: 13px;">
									...
									<td style="width:50px;">角色组:</td>
										...
										<c:forEach items="${roleList}" var="role" varStatus="vs">
											<td style="width:100px;" class="center" <c:choose><c:when test="${pd.ROLE_ID == role.ROLE_ID}">bgcolor="#FFC926" onMouseOut="javascript:this.bgColor='#FFC926';"</c:when><c:otherwise>bgcolor="#E5E5E5" onMouseOut="javascript:this.bgColor='#E5E5E5';"</c:otherwise></c:choose>  onMouseMove="javascript:this.bgColor='#FFC926';" >
												<a href="buttonrights/goEdit.do?ROLE_ID=${role.ROLE_ID }&FHBUTTON_ID=${buttonObj.FHBUTTON_ID}" style="text-decoration:none; display:block;"><i class="menu-icon fa fa-users"></i><font color="#666666">${role.ROLE_NAME }</font></a>
											</td>
							...
							<table id="dynamic-table" class="table table-striped table-bordered table-hover" style="margin-top:7px;">
								...
								<tr>
									...
									<c:if test="${QX.edit == 1 }">
											<th class='center'>${buttonObj.NAME }</th>
									</c:if>
								</tr>
								</thead>
								<c:choose>
										...
										<c:if test="${QX.edit == 1 }">
											<!-- 按钮循环 -->
												...
												<c:forEach items="${roleFhbuttonlist}" var="varRb" varStatus="vsRb">
													<c:if test="${var.ROLE_ID == varRb.ROLE_ID && buttonObj.FHBUTTON_ID == varRb.BUTTON_ID }">
														<c:set value="1" var="rbvalue"></c:set>
													</c:if>
												</c:forEach>
												<td class='center' style="height: 20px;">
													<label>
														<input name="switch-field-1" onclick="upRb('${var.ROLE_ID}','${buttonObj.FHBUTTON_ID}')" class="ace ace-switch ace-switch-3" type="checkbox" <c:if test="${rbvalue == 1 }">checked="checked"</c:if> >
														<span class="lbl"></span>
													</label>
												</td>
